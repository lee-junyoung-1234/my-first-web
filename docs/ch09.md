...
로그인 페이지와 회원가입 페이지는 사용자가 인증 기능을 실제로 만나는 입구다. 이 화면들은 Supabase 로직을 직접 길게 쓰기보다 9.6에서 만든 `lib/auth.ts` 함수를 호출하는 역할만 맡는다. 그래서 화면 파일은 입력값 관리, 버튼 클릭, 성공 후 이동, 실패 메시지 표시 정도로 단순하게 유지한다.# Chapter 9. Supabase Authentication
